...
|
| 25428-4    | Estradiol (E2)                                        | Male: 10-50, Female:   21-312 (premenopausal), 0-178 (postmenopausal) | pg/mL               
|
...
|
| 55277-8    | Pain severity -                                        | N/A (subjective scale)         | NRS, VAS or others
|
...
|
| 2947-0     | Direct Antiglobulin Test (DAT) - IgG                   | Negative                       | (dimensionless)
|
| 14804-9    | Direct Antiglobulin Test (DAT) - IgA                   | Negative                       | (dimensionless) 
|
| 44963-7    | Direct Antiglobulin Test (DAT) - IgM                   | Negative                       | (dimensionless)
|
...
|
| 21000-5    | Urine specific gravity                                 | 1.005-1.030                    | (dimensionless)
|
...
|
| 5799-2     | Albumin/Globulin Ratio (A/G) (V3)                      | 1.1-2.5                        | (dimensionless)
|
...
|
| 92142-9    | von Willebrand Factor Multimers                        | N/A (patterns interpretation)  | (dimensionless)
|
...
|
| 92138-7    | von Willebrand Factor Antigen/Activity ratio (vWF:R)   | 0.6-2.0                         | (dimensionless)
|
...
|
| 85318-4    | Immature Granulocytes Absolute (IG)_#/L                | 0-100                           | /µL (cells/µL)
|
...
|
| 751-8      | Blood loss (screening)                                 | Negative                        | (dimensionless)
|
...
|
| 80382-5    | Sodium (in)/Potassium (out) Concentration Difference   | N/A (Difference used for interpretation)| mEq/L 
|
...
|
| 6301-6     | WBC White Blood Corpus_DSCH_PLN                        | 4,500-11,000                    | /µL (cells/µL)
|
...
|
| 2951-2     | Direct Antiglobulin Test (DAT) - C3b                   | Negative                        | (dimensionless)
|
...
# This regex pattern will match Markdown table rows, capturing the content within the '|' characters
pattern = re.compile(r'\|([^\|]+?)\|([^\|]+?)\|([^\|]+?)\|([^\|]+?)\|')
matches = pattern.findall(markdown_table.strip())

# Write the parsed data to a CSV file
with open('output.csv', 'w', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    for row in matches:
        csv_writer.writerow([cell.strip() for cell in row])